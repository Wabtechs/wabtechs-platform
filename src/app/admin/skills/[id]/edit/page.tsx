  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/admin/skills", {
        ...
        body: JSON.stringify({ id, ...form, percent: Number(form.percent), order: Number(form.order) }),
      });
  ...
  return (
    <div className="pt-24 pb-16">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-foreground">Modifier la compétence</h1>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          <Card className="border-gray-200 bg-white dark:border-border dark:bg-card">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label className="text-gray-900 dark:text-foreground">Pourcentage</Label>
                <Input
                  type="number"
                  value={form.percent}
                  onChange={(e) => updateForm("percent", e.target.valueAsNumber || 0)}
                  min={0}
                  max={100}
                  className="border-gray-200 bg-gray-50 text-gray-900 dark:border-border dark:bg-muted dark:text-foreground"
                />
              ...
              <div className="space-y-2">
                <Label className="text-gray-900 dark:text-foreground">Image (URL)</Label>
                <Input
                  value={form.image}
