...
export default async function OsBusinessPage({ searchParams }: { searchParams: Promise<{ project?: string }> }) {
  const { project } = await searchParams;
  const projects = await db.osProject.findMany({ select: { id: true, slug: true, name: true, color: true }, orderBy: { name: "asc" } });
  ...
  const revenueProjects = allProjects.filter((p) => Number(p.mrr) > 0).sort((a, b) => Number(b.mrr) - Number(a.mrr));
  ...
  return (
    <div>
      ...
      >
        <OsProjectFilter projects={projects} value={selected?.slug ?? "all"} basePath="/admin/os/business" />
      </OsPageHeader>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <OsStatCard label="MRR cumulé" value={fmtEur(totalMrr)} hint="Revenu mensuel récurrent" icon={<Euro className="h-4 w-4" />} accent="text-emerald-500" />
        <OsStatCard label="Projets monétisés" value={`${revenueProjects.length}/${allProjects.length}`} hint="Générant du revenu" icon={<Wallet className="h-4 w-4" />} accent="text-blue-500" />
        <OsStatCard label="Stars cumulées" value={fmtNum(totalStars)} hint="Traction GitHub" icon={<Star className="h-4 w-4" />} accent="text-amber-500" />
        <OsStatCard label="Top projet" value={revenueProjects[0]?.name ?? "—"} hint={revenueProjects[0] ? `${fmtEur(revenueProjects[0].mrr)} MRR` : "Aucun revenu"} icon={<TrendingUp className="h-4 w-4" />} accent="text-primary" />
      </div>
      ...
      <div className="mt-8 grid gap-4 lg:grid-cols-2">
        <Card className="border-border bg-card">
          ...
          <CardContent>
            {revenueProjects.length === 0 ? (
              <OsEmpty title="Aucun revenu" hint="Aucun projet ne génère de revenu pour le moment." />
            ) : (
              <div className="space-y-4">
                {revenueProjects.map((p) => {
                  const max = revenueProjects[0]?.mrr;
                  const pctVal = max && Number(max) > 0 ? Math.max(4, Math.round((Number(p.mrr) / Number(max)) * 100)) : 0;
                  return (
                    <div key={p.id} className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 h-2 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-white/10">
                          <div className="h-full rounded-full" style={{ width: `${pctVal}%`, background: p.color }} />
                        </div>
                      </div>
                    </div>
                  );
        ...
        <Card className="border-border bg-card">
          ...
          <CardContent>
            {growth.length ? (
              <OsLineChart data={growth} xKey="label" height={240} lines={[{ key: "mrr", name: "MRR (€)", color: "#10b981" }]} />
            ) : (
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <Card className="border-border bg-card">
          ...
          <CardContent>
            {growth.length ? (
              <OsLineChart data={growth} xKey="label" height={220} lines={[
        ...
        <Card className="border-border bg-card">
          ...
          <CardContent>
            {growth.length ? (
              <OsLineChart data={growth} xKey="label" height={220} lines={[{ key: "stars", name: "Stars", color: "#842ae3" }]} />
            ) : (
              <OsEmpty title="Aucune donnée" />
            )}
          </CardContent>
        ...
        <Card className="border-border bg-card">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="mt-2 flex h-2.5 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-white/10">
              {allProjects.map((p) => (
                <span key={p.id} style={{ width: `${totalMrr > 0 ? (Number(p.mrr) / totalMrr) * 100 : 0}%`, background: p.color }} />
              ))}
            </div>
          </CardContent>
        </Card>
