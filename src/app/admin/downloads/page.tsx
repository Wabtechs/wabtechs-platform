...
import { DeleteDownloadButton } from "./delete-button";

export const metadata: Metadata = { title: "Gestion des téléchargements" };
...
export default async function AdminDownloadsPage({
  ...
  return (
    <div>
        ...
        {items.length === 0 ? (
          <Card className="border-border bg-card">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-[13px] text-gray-500">Aucun téléchargement</p>
              <Link
                href="/admin/downloads/new"
                className="mt-2 text-[12px] font-medium text-primary hover:underline"
              >
                Créer votre premier téléchargement
              </Link>
            </CardContent>
          </Card>
        ) : (
      ...
      <PaginationLinks currentPage={currentPage} totalPages={totalPages} basePath="/admin/downloads" />
    </div>
  );
}
