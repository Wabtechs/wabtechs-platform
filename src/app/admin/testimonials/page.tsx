...
export const metadata: Metadata = { title: "Gestion des témoignages" };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[13px] text-gray-500 dark:text-gray-400">
                {items.length} témoignages au total
              </p>
            </div>
          </div>
        </div>
        ...
        {items.length === 0 ? (
          <Card className="border-gray-200/80 bg-white dark:border-border dark:bg-card">
            <CardContent className="flex flex-col items-center justify-center py-16">
              <MessageCircle className="mb-3 h-8 w-8 text-gray-300 dark:text-gray-600" />
              <p className="text-[13px] text-gray-500">Aucun témoignage</p>
              <Link
                href="/admin/testimonials/new"
                className="mt-2 text-[12px] font-medium text-primary hover:underline"
              >
                Créer votre premier témoignage
              </Link>
            </CardContent>
          </Card>
        ) : (
          items.map((item) => (
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[12px] text-gray-400 line-clamp-1">
                  {item.text}
                </p>
              </div>
