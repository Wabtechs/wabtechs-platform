...
import { DeleteRoadmapButton } from "./delete-button";

export const metadata: Metadata = { title: "Gestion des roadmaps" };
export const dynamic = "force-dynamic";

const statusStyles: Record<string, string> = {
  planned: "bg-gray-100 text-gray-500 dark:bg-white/5 dark:text-gray-400",
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[13px] text-gray-500 dark:text-gray-400">
                {items.length} roadmaps au total
              </p>
            </div>
          </div>
        </div>
        ...
        {items.length === 0 ? (
          <Card className="border-gray-200/80 bg-white dark:border-border dark:bg-card">
            <CardContent className="flex flex-col items-center justify-center py-16">
              <Map className="mb-3 h-8 w-8 text-gray-300 dark:text-gray-600" />
              <p className="text-[13px] text-gray-500">Aucune roadmap</p>
              <Link
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${statusStyles[item.status] ?? statusStyles.planned}`}
                  >
                ...
                <div className="mt-1.5 flex items-center gap-2 text-[12px] text-gray-400">
                  <span>{item.slug}</span>
                  <span>·</span>
                  <span>{item.quarter ?? "N/A"} {item.year ?? ""}</span>
                  <span>·</span>
              ...
              <div className="ml-4 flex items-center gap-1">
                ...
                >
                  <Link href={`/admin/roadmaps/${item.id}/edit`}>
                    <Pencil className="h-3.5 w-3.5" />
                  </Link>
                </Button>
                <DeleteRoadmapButton id={item.id} />
              </div>
            </div>
          ))
        )}
