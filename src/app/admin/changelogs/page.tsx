...
export const metadata: Metadata = { title: "Gestion du changelog" };
...
export default async function AdminChangelogsPage({
  ...
  return (
    <div>
        ...
        {items.length === 0 ? (
          <Card className="border-border bg-card">
            <CardContent className="flex flex-col items-center justify-center py-16">
              <GitCompare className="mb-3 h-8 w-8 text-gray-300 dark:text-gray-600" />
              <p className="text-[13px] text-gray-500">Aucun élément</p>
              <Link
                href="/admin/changelogs/new"
                className="mt-2 text-[12px] font-medium text-primary hover:underline"
              >
                Créer votre premier élément
              </Link>
            </CardContent>
          </Card>
        ) : (
          items.map((item) => (
            ...
            >
              ...
              <div className="ml-4 flex items-center gap-1">
                ...
                >
                  <Link href={`/admin/changelogs/${item.id}/edit`}>
      ...
      <PaginationLinks currentPage={currentPage} totalPages={totalPages} basePath="/admin/changelogs" />
    </div>
  );
}
